...
/** Filing Workspace (P7) — escalate → affidavit → record → release. */
  ...
  return (
    <>
      <div className="flex items-center gap-3 rounded-lg px-4 py-3" style={{ background: alpha("#eb143f", 0.07), border: `1px solid ${alpha("#eb143f", 0.28)}` }}>
        <AlertTriangle className="h-[18px] w-[18px] shrink-0 text-error" />
        <div className="text-[13px] text-text">Compliance check: <span className="font-semibold text-error">1 monthly notice was sent late</span> — review before generating the affidavit.</div>
      </div>
      {STEPS.map((f) => (
        <div key={f.n} className="flex gap-3.5 rounded-lg border border-border bg-surface p-[18px]">
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md font-mono text-[13px] font-bold" style={{ color: f.color, background: alpha(f.color, 0.14) }}>{f.n}</span>
          <div className="min-w-0 flex-1">
            <div className="text-[13.5px] font-semibold text-text">{f.title}</div>
            <div className="mt-0.5 text-[12px] leading-snug text-text-dim">{f.body}</div>
          </div>
          <span className="self-center whitespace-nowrap rounded-sm px-2 py-0.5 text-[10.5px] font-bold" style={{ color: f.color, background: alpha(f.color, 0.14) }}>{f.st}</span>
        </div>
      ))}
    </>
  );
