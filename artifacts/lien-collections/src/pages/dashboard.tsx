...
export default function DashboardPage() {
  ...
  const inCollections = accounts.filter((a) => a.status === "in_collections").length;
              ...
              <Link key={d.id} href={`/projects/${d.id}`}>
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 text-[11.5px]" style={{ color: "var(--text-muted-color)" }}>
                      Lien stream active
                    </div>
                  </div>
          ...
          {topAccounts.length > 0 && (
            <>
              ...
              <div className="space-y-1.5">
                {topAccounts.map((a) => {
                  const rc = (a.riskScore ?? 0) >= 75 ? "#eb143f" : (a.riskScore ?? 0) >= 45 ? "#f59f0a" : "#14eba3";
                  return (
